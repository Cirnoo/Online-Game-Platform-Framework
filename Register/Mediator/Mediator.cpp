...
/************************************************************************/
	...
	((pCPNGButton)pControlBase)->Create(rec,pParentWnd,IDC_MIN,sys.vec_bt_min[0],sys.vec_bt_min[1],CMD_MINSIZE);
	vec_control.push_back(pControlBase);

	rec=Rect(mWidth-38,0,c_width+10,c_height);
	pControlBase = new CPNGButton;
	((pCPNGButton)pControlBase)->Create(rec,pParentWnd,IDC_CLOSE,sys.vec_bt_close[0],sys.vec_bt_close[1],CMD_CLOSE);
	vec_control.push_back(pControlBase);
